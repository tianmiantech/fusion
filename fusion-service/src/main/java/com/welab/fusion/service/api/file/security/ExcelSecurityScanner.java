 ...
 * @date 2021/12/31
 */
public class ExcelSecurityScanner extends FileSecurityScanner {

    @Override
    protected void doScan(File file) throws IOException {
        ...
        try (ExcelReader reader = new ExcelReader(file)) {
            ...
            for (int i = 0; i < totalRowCount; i++) {
                List<Object> row = reader.getRowDataWithoutLastEmptyCell(0, i);
                if (row == null) {
                    continue;
                }
                checkRow(row);
    ...
    private void checkRow(List<Object> row) throws IOException {
        for (Object item : row) {
