...
public class SM4Util {
     ...
     * @explain Encryption mode：CBC
     */
    public static String encrypt(String hexKey, String plaintext, AlgorithmParameters iv) throws Exception {
        byte[] keyData = ByteUtils.fromHexString(hexKey);
        byte[] srcData = plaintext.getBytes(ENCODING);
    ...
    public static String encryptBase64(String hexKey, String plaintext) throws Exception {
        ...
        byte[] srcData = plaintext.getBytes(ENCODING);
        byte[] cipherArray = encryptCbcPadding(keyData, srcData);
        return cn.hutool.core.codec.Base64.encode(cipherArray);
    }
    ...
    public static String decryptBase64(String hexKey, String ciphertext) throws Exception {
        byte[] keyData = ByteUtils.fromHexString(hexKey);
        byte[] resultData = Base64.decode(ciphertext);
        byte[] srcData = decryptCbcPadding(keyData, resultData);
        return new String(srcData, ENCODING);
    }
     ...
     * CBC in encryption mode
     */
    private static byte[] encryptCbcPadding(byte[] key, byte[] data, AlgorithmParameters iv) throws Exception {
    ...
    private static Cipher generateCbcCipher(String algorithmName, int mode, byte[] key) throws Exception {
        return generateCbcCipher(algorithmName, mode, key, generateIV());
    }

    public static Cipher generateCbcCipher(String algorithmName, int mode, byte[] key, AlgorithmParameters iv) throws Exception {
