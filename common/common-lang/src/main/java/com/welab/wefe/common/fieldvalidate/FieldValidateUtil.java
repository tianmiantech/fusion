    ...
    public static void checkAndStandardize(Object obj) throws StatusCodeWithException, IllegalAccessException {
        ...
        for (Field field : totalFields) {
            ...
            if (value instanceof String) {
                ...
                blockSqlInjection(check, obj, field, valueStr);

                // sql 安全性检查
                oneSelectSql(check, valueStr);
                multiSelectSql(check, valueStr);
            }

        }
    }

    private static void multiSelectSql(Check check, String valueStr) throws StatusCodeWithException {
        if (!check.multiSelectSql()) {
            return;
        }

        Validator.mustBeMultiSelectSql(valueStr);
    }

    private static void oneSelectSql(Check check, String valueStr) throws StatusCodeWithException {
        if (!check.oneSelectSql()) {
            return;
        }

        Validator.mustBeOneSelectSql(valueStr);
    }

    /**
     * 阻止 sql 注入
     */
    private static void blockSqlInjection(Check check, Object obj, Field field, String valueStr) throws IllegalAccessException {
        if (!check.blockSqlInjection()) {
            return;
        }
        field.set(
                obj,
