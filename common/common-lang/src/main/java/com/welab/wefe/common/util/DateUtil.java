...
public class DateUtil {
    ...
    public static final String Y4_M2_D2_H2_M2_S2_SSS2 = "yyyyMMddHHmmssSSS";
     ...
     * @return The number of days between two times
     */
    public static Long intervalHours(long timeInMillis1, long timeInMillis2, boolean isAbs) {
        LocalDateTime time1 = LocalDateTime.ofEpochSecond(timeInMillis1 / 1000, 0, ZoneOffset.ofHours(8));
        LocalDateTime time2 = LocalDateTime.ofEpochSecond(timeInMillis2 / 1000, 0, ZoneOffset.ofHours(8));
        return isAbs ? Duration.between(time1, time2).abs().toHours() : Duration.between(time2, time1).toHours();
    }
     ...
     * 设置日期时、分、秒、毫秒
     */
    public static Date setDateHMSMill(Date date, int hour, int minute, int second, int millisecond) {
        if (null == date) {
            return null;
        }
        Calendar calendar = GregorianCalendar.getInstance();
